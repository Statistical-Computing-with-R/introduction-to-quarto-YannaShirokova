...
*The two supplement delivery methods include OJ (orange juice) and VC (ascorbic acid -- form of vitamin C).*
...
#| fig-subcap: 
#|   - "Tooth Length v.s. Supplement" 
#|   - "Tooth Length v.s. Dose Supplement"
...
# T-test for mean tooth length for  OJ supplement vs VC supplement methods

```{r}
#| echo: true

data(ToothGrowth)
results <- t.test(len ~ supp, data = ToothGrowth, 
...
1. The p value is 0.06063, which is greater than .05, thus I fail to reject that the treatment mean tooth length for the OJ supplement delivery method is the same as the treatment mean tooth length for the VC supplement delivery method (null hypothesis).
2. We are 95% confident that true difference in mean tooth length between the OJ and VC groups lies between -0.1710156 and 7.5710156 mm.